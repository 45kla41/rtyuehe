...
@app.route('/greeting/<username>') # параметр запроса username (строка)
def greeting(username): # username - параметр функции обработчика запроса 
    return f'''<!doctype html> # заголовок страницы
                <html lang="en">
                  <head>
                    <meta charset="utf-8">
                    <title>Привет, {username}</title> # параметр, который пришел помещен в заголовок стр.
                  </head>
                  <body> # тело страницы
                    <h1>Привет, {username}!</h1> # параметр, который пришел помещен в тело стр.
                  </body>
                </html>'''

@app.route('/getnumber/<int:number>') # параметр запроса number (число)
def get_number(number):
    number += 1
    return f'''<!doctype html> # заголовок страницы содержит 
                <html lang="en">
                  <head>
                    <meta charset="utf-8">
                  </head>
                  <body> # тело страницы
                    <h1>Запрошено число {number}!</h1> # заголовок первого уровня (h1)
                  </body>
                </html>'''
